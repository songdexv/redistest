...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    protected static Logger logger = Logger.getLogger(ShardedJedisSentinelPool.class.getName());
    ...
    /**
     * @param masters   master names
     * @param sentinels sentinel addr ip:port
     */
    ...
    private List<HostAndPort> initSentinels(Set<String> sentinels, List<String> masters) {
            ...
            while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
                for (String sentinel : sentinels) {
                    ...
                    try {
                        ...
                        logger.severe("Cannot connect to sentinel running @ " + hap + ". Trying next one.");
                    }
                }
                if (null == master) {
                    try {
                        logger.severe("All sentinels down, cannot determine where is "
        ...
        if (masters.size() != 0 && masters.size() == shardMasters.size()) {
            ...
            logger.severe("not all shard master reachable");
            throw new IllegalStateException("all shard masters should be reachable");
        }
        return shardMasters;
    }
    ...
    private List<JedisShardInfo> makeShardInfoList(List<HostAndPort> masters) {
        ...
        for (HostAndPort master : masters) {
            JedisShardInfo shard = new JedisShardInfo(master.getHost(), master.getPort(), timeout);
            shard.setPassword(password);
            shardMasters.add(shard);
        }
        return shardMasters;
    }
    ...
    protected static class ShardedJedisFactory implements PooledObjectFactory<ShardedJedis> {
        ...
        public void destroyObject(PooledObject<ShardedJedis> p) throws Exception {
            final ShardedJedis shardedJedis = p.getObject();
        ...
        public boolean validateObject(PooledObject<ShardedJedis> p) {
            try {
                ShardedJedis shardedJedis = p.getObject();
                for (Jedis jedis : shardedJedis.getAllShards()) {
    ...
    protected class MasterListener extends Thread {
        ...
        public void run() {
            ...
            while (running.get()) {
                ...
                try {
                    ...
                    jedis.subscribe(new JedisPubSubAdapter() {
                        ...
                        public void onMessage(String channel, String message) {
                            ...
                            if (switchMasterMsg.length > 3) {
                                ...
                                logger.severe("Invalid message received on Sentinel "
                    ...
                    if (running.get()) {
                        logger.severe("Lost connection to Sentinel at " + host
                                + ":" + port
        ...
        public void shutdown() {
            try {
                ...
                logger.severe("Caught exception while shutting down: " + e.getMessage());
            }
        }
    }
}
